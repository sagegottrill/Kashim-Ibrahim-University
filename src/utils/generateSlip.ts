...
export const generateSlip = async (data: ApplicationData, returnData: boolean = false): Promise<string | void> => {
    ...
    const loadImage = async (url: string): Promise<string> => {
        ...
        // If we are on www.kiuth.com.ng but image is on kiuth.com.ng (or vice versa),
        // we can rewrite the image URL to match the current origin to make it a Same-Origin request.
        try {
            const currentUrl = new URL(window.location.href);
            const imageUrl = new URL(url);

            // Check if they are the same base domain (e.g., kiuth.com.ng)
    ...
    try {
        ...
        doc.text("Kashim Ibrahim University", 50, 18);
        doc.setFontSize(16);
        doc.text("Teaching Hospital", 50, 28);

        doc.setFontSize(10);
        doc.setTextColor(200, 200, 200);
        doc.text("Recruitment Portal", 195, 35, { align: 'right' });

        // 4. Title Section
        doc.setFontSize(24);
        doc.setTextColor(BRAND_BLUE);
        doc.text("Application Slip", 105, 60, { align: "center" });
        ...
        addRow("Reference Number", data.reference_number, true);
        addRow("Full Name", data.full_name);
        addRow("Date of Birth", data.date_of_birth, true);
        addRow("State of Origin", data.state_of_origin);
        addRow("Position Applied", data.position, true);
        addRow("Department", data.department);
        addRow("Date Applied", new Date().toLocaleDateString(), true);
        ...
        doc.setTextColor(100, 100, 100);
        doc.setFontSize(9);
        doc.text("This slip serves as proof of your application.", 70, qrY + 25);
        doc.text("Please present this document at the screening venue.", 70, qrY + 32);
        doc.text(`Generated on: ${new Date().toLocaleString()}`, 70, qrY + 42);

        // 8. Footer
        doc.setFontSize(8);
        doc.setTextColor(150, 150, 150);
        doc.text("Kashim Ibrahim University Teaching Hospital Recruitment Portal", 105, 285, { align: "center" });

        if (returnData) {
            const dataUri = doc.output('datauristring');
            return dataUri.split(',')[1];
        } else {
            doc.save(`KIUTH_Slip_${data.reference_number}.pdf`);
        }
